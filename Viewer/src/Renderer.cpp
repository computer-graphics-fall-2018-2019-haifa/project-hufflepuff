...
void Renderer::DrawModel(const Scene& scene, MeshModel* _model, glm::mat4 transMat) {
	MeshModel model = *_model;
	glm::mat4 modelMat = scene.GetWorldTransformation() * model.GetWorldTransformation();
	glm::mat4 viewMat = scene.GetActiveCamera().GetViewTransformation();
	glm::mat4 projMat = scene.GetActiveCamera().GetProjTransformation();

	// Set the uniform variables
	colorShader.setUniform("model", modelMat);
	colorShader.setUniform("view", viewMat);
...
void Renderer::Render(const Scene& scene)
{
	...
	//		glm::mat4 transMatrix = Utils::TransMatricesModel(scene, currentModelIndex);
	//		//glm::mat4 transMatrix = Utils::GetScaleMatrix(currentModel.scale);

	//		// Activate the 'colorShader' program (vertex and fragment shaders)
	//		colorShader.use();

	//		// Set the uniform variables
	//		colorShader.setUniform("model", transMatrix);
	//		colorShader.setUniform("view", glm::mat4(1));
	//		colorShader.setUniform("projection", glm::mat4(1));
	//		colorShader.setUniform("material.textureMap", 0);
	...
	//		colorShader.setUniform("color", currentModel.color);
