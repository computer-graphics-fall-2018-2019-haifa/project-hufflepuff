...
//-----------------------------------------------------------------------------
string ShaderProgram::fileToString(const string& filename) const
{
	std::stringstream ss;
	std::ifstream file;
...
//-----------------------------------------------------------------------------
void  ShaderProgram::checkCompileErrors(GLuint shader, ShaderType type) const
{
	int status = 0;
...
{
	GLint loc = getUniformLocation(name);
	glUniform4fv(loc, 5, glm::value_ptr(v[0]));
}
